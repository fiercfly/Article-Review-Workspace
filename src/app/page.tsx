...
export default function Home() {
  ...
  if (isLoading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-slate-500">
        <div className="w-8 h-8 border-2 border-slate-900 dark:border-white border-t-transparent rounded-full animate-spin mb-3" />
        <p className="text-xs font-medium">Loading workspace...</p>
      </div>
    );
  }

  // Welcome Screen (Unauthenticated - Mobile First Minimalist)
  if (!user) {
    return (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {allUsers.map((u) => {
              ...
              if (isAlice) {
                ...
                badgeColor = "bg-indigo-50 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800";
                desc = "Full access to Projects Alpha & Beta. Can import articles.";
              } else if (!isAlice && !isBob) {
                title = "Dr. Charlie (External)";
                badgeColor = "bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800";
              ...
              return (
                <button
                  key={u.id}
                  onClick={() => login(u.id)}
                  className="group text-left p-4 sm:p-5 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-indigo-500 dark:hover:border-indigo-500 transition flex flex-col justify-between h-44 bg-slate-50/50 dark:bg-slate-850/50"
                >
                  <div>
                    <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded ${badgeColor}`}>
                      {title.split(" ")[0]}
                    </span>
                    <h3 className="font-bold text-sm text-slate-900 dark:text-white mt-3 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                      {u.name.split(" (")[0]}
                    </h3>
                  ...
                  <div className="flex items-center text-xs font-semibold text-indigo-600 dark:text-indigo-400 mt-4">
  ...
  // Dashboard (Authenticated - Mobile First Minimalist)
  ...
  return (
    <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Welcome Banner */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 mb-8 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">
            Welcome, {user.name.split(" (")[0]}
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
            Organization: <strong className="text-slate-800 dark:text-slate-200">{user.orgMemberships[0]?.organization.name || "Workspace"}</strong>
          </p>
        </div>
        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg text-xs font-semibold self-start sm:self-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Projects list (Mobile First Grid) */}
        <div className="lg:col-span-2 space-y-4">
          ...
          {user.projectMemberships.length === 0 ? (
            <div className="p-8 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl">
              ...
              <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">No assigned projects</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {user.projectMemberships.map((pm) => (
                <Link
                  ...
                  className="group block p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-indigo-500 dark:hover:border-indigo-500 rounded-xl transition shadow-sm hover:shadow-md"
                >
                  ...
                  <div className="flex items-center text-xs font-semibold text-indigo-600 dark:text-indigo-400 mt-4 pt-3 border-t border-slate-100 dark:border-slate-800">
        ...
        {/* Sidebar Controls / Create Project (Mobile First Stack) */}
        <div className="space-y-6">
              ...
              <form onSubmit={handleCreateProject} className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 resize-none"
                  />
                </div>
                ...
                {createError && (
                  <div className="p-2.5 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-lg text-xs text-red-600 dark:text-red-400 flex items-center gap-1.5">
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-slate-900 dark:bg-indigo-600 hover:bg-slate-800 dark:hover:bg-indigo-500 text-white rounded-lg text-xs font-semibold transition disabled:opacity-50"
                >
          ...
          <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-4 text-xs text-slate-500 dark:text-slate-400 space-y-2">
